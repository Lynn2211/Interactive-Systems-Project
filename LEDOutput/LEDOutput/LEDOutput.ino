...
#define CLEAR_PIN 14
#define SERIALDATA_PIN 2
#define SHIFTCLOCK_PIN 0
#define LATCHCLOCK_PIN 4
...
void setup() {
  ...
  digitalWrite(CLEAR_PIN, LOW); //Clear the shift register
...
void lightUpLeds(int value){
  ...
  //Take latch clock low so LEDs don't change while sending
  digitalWrite(LATCHCLOCK_PIN, LOW);

  //Least significant bit first to the LEDs
  shiftOut(SERIALDATA_PIN, SHIFTCLOCK_PIN, LSBFIRST, value);

  //Take latch high so LEDs change
